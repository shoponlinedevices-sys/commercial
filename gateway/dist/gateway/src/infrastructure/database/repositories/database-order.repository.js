...
let DatabaseOrderRepository = class DatabaseOrderRepository {
    ...
    async findAll() {
        ...
        return orders.map((order) => new order_entity_1.Order(order.id, order.userId, parseFloat(order.totalPrice || '0'), this.mapStatusToString(order.status || 1), order.createdAt, order.updatedAt));
    }
    async updateStatus(orderId, status) {
        ...
        return new order_entity_1.Order(savedOrder.id, savedOrder.userId, parseFloat(savedOrder.totalPrice || '0'), this.mapStatusToString(savedOrder.status || 1), savedOrder.createdAt, savedOrder.updatedAt);
    }
